...
- 不得把 Qwen Visual Evidence Mode 扩展成代码 Agent。Qwen 只能作为视觉理解 provider，不得读取源码、修改文件、运行命令或接收 secret/token/证书/私钥/完整源码/图片 bytes/base64。
- v6.0 首选实战形态是 reference-guided migration：用户把源 App 参考截图放入目标仓库的 `visual_validation.reference_screenshot_dirs`，Qwen 只读这些截图并输出视觉指导，DeepSeek/主 Agent 才能改代码。
- 不得把 reference-only 视觉指导当成完整真实渲染验收；报告和 PR body 必须区分 `guidance_completed` 与 `full_rendered_validation`。
...
- v6.0 视觉模式闭环：`docs/QWEN_VISUAL_MODE.md`、`skills/qwen_visual_mode.md`、`agent/forgis_config.py` 中的 `visual_validation`、`agent/visual_evidence.py`、`agent/qwen_vision.py`、`agent/file_tools.py` 中的 `list_visual_references` / 三个 inspect/compare 工具分发、`agent/runtime_controller.py` 中的视觉状态/gate、`agent/run_report.py` / `agent/pr_body.py` 中的视觉摘要。当前已支持 reference-guided migration、受控视觉工具、mock-first Qwen provider、显式 env 下的真实 Qwen HTTP transport、report/PR 字段和防假验收 gate；仍不包含自动截图、artifact 上传、多 provider 或任意 shell。
...
- `docs/QWEN_VISUAL_MODE.md`：Qwen Visual Evidence Mode 的 reference-guided workflow、provider 边界、reference-first 规则、证据状态、报告字段、runtime gate、真实 transport 启用条件和当前仍不实现的截图/上传/多 provider 项。
