...
`FORGIS_CONFIG.yml` 必须放在目标仓库根目录，必须是非空 YAML mapping，并且只能使用 Forgis 当前支持的字段。未知字段会在 Resolve Forgis config 阶段直接失败，模型不会开始运行。

请把三类信息分开：

- **GitHub Actions input / CLI 提供，不能写进 config：** `target_repo`。
- **`FORGIS_CONFIG.yml`：** repo ref、输出分支、输出子目录、任务文件路径、DeepSeek 连接字段、运行开关、skills、report、repair loop、migration plan，以及不含 secret 的 visual validation 开关。
...
- `agent_backend: aider`：Forgis 当前只支持 `agent_backend: deepseek`。
...
- `FORGIS_CONFIG.yml` 中不得写 Qwen API key、token、image path、evidence root 或 screenshot path。v6.0 只接受下文记录的不含 secret 的 `visual_validation` 控制块；Qwen key/base/model 只能通过显式 runtime env 提供，且不会写入报告。
...
### DeepSeek model 与 secret

DeepSeek 模型应使用 API 接受的短 id：
...
这里写的是环境变量名，不要把真实 API key 写进 `FORGIS_CONFIG.yml`。

### Qwen Visual Evidence Mode（v6.0 闭环）

Forgis v6.0 已接通 Qwen Visual Evidence Mode 闭环：`docs/QWEN_VISUAL_MODE.md`、本地 skill `skills/qwen_visual_mode.md`、可选 `visual_validation` 配置块的严格解析、`agent/visual_evidence.py` 证据路径/状态 helper、可 mock 且支持安全真实 HTTP transport 的 `agent/qwen_vision.py` provider adapter、三个视觉工具（`inspect_visual_reference`、`inspect_visual_actual`、`compare_visual_screenshots`）、sandbox dispatch、runtime visual state、run report / PR body 视觉摘要，以及防止假视觉验收的 runtime gate。由于 `visual_validation` 已成为稳定顶层 report 块，run report schema 当前为 `forgis.run_report.v6.0`。

Qwen 的定位是视觉理解 provider，不是代码迁移 Agent。主 Agent 仍负责读源码、改代码、构建、测试和最终报告。视觉模式遵循 reference-first：允许 reference-only 检查，但它不是完整真实渲染对比。不得向 Qwen 发送源码、secret、token、`.env`、证书、私钥、provisioning profile 或私有本地配置。单元测试默认 mock provider 且不联网；只有显式提供 `QWEN_API_KEY` 时才会真实调用 Qwen HTTP transport，可选 `QWEN_API_BASE` 和 `QWEN_VISION_MODEL`。当前仍不实现自动截图、visual artifact 上传、多 provider 或 UI dashboard；这些留给 Phase 8+。

`visual_validation.enabled=auto` 使用确定且保守的规则：显式选中 `qwen_visual_mode`、任务文本包含强 UI/视觉/截图关键词，或任一视觉工具已被调用时才要求视觉证据。纯代码、后端、配置、构建脚本或单元测试修复不会自动进入视觉 gate，除非出现上述信号。
